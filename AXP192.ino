...
// Return True = Battery Exist
bool AXP192::GetOverTempWarning() {
    if (Read8bit(0x01) & 0b10000000)
        return true;
    else
        return false;
}
...
// Not recommend to set charge current > 100mA, since Battery is only 80mAh. // This was for another board, a 18650 is beefier
// more then 1C charge-rate may shorten battery life-span.
/*
Only last four bits matters:
0000:100mA
0001:190mA
0010:280mA
0011:360mA
...
void AXP192::SetChargeCurrent(uint8_t current) {
    uint8_t buf = Read8bit(0x33);
    buf         = (buf & 0xf0) | (current & 0x0F); //(current & 0x07);
    Write1Byte(0x33, buf);
}
